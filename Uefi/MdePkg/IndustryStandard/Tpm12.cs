...
typedef struct tdTPM_IDENTITY_PROOF
{
  TPM_STRUCT_VER ver;
  uint labelSize;
  uint identityBindingSize;
  uint endorsementSize;
  uint platformSize;
  uint conformanceSize;
...
typedef struct tdTPM_DA_INFO
{
  ...
  TPM_DA_STATE state;
  ushort currentCount;
  ushort thresholdCount;
  TPM_DA_ACTION_TYPE actionAtThreshold;
  uint actionDependValue;
  uint vendorDataSize;
  byte* vendorData;
}
TPM_DA_INFO;
